...
def point_over_time(ori_data, rom_data_0, rom_data_1, rom_data_2,pointNo, fieldName,startNumber,led_location):
 # rom_data_2, rom_data_4, rom_data_5, pointNo, fieldName):# 1: full model; 2: ROM model
    ...
    if fieldName == 'Velocity': # flow_past_cylinder
        ...
        plt.legend(['Full Model', 'SAE-LSTM m=13', 'SAE-LSTM m=33', 'SAE-LSTM m=66'], fontsize=19,loc=led_location,frameon=True)
        ...
        # ax0.plot(x, y, x, y_0, x, y_1, x, y_2, x, y_3, linewidth = 0.8)
        ax0.plot(x, y, x, y_0, x, y_1, x, y_2, linewidth = 1.0)
        ...
        plt.legend(['Full Model', 'SAE-LSTM m=7', 'SAE-LSTM m=12', 'SAE-LSTM m=35'], loc=led_location,fontsize=14)
...
def point_over_time_error(ori_data, rom_data_0, rom_data_1, rom_data_2, pointNo, fieldName, startNumber,
    ...
    if fieldName == 'Velocity':  # flow_past_cylinder
        ...
        # plt.legend(['Full Model', 'ROM(AE+TF)', 'ROM(PCA+TF)'], loc='lower right')
        plt.legend(['SAE-LSTM m=13', 'SAE-LSTM m=33', 'SAE-LSTM m=66'], fontsize=18,
                   loc=led_location)
        ...
        ax0.set_prop_cycle(color=['#2f3542', '#f6b93b', '#FF3030', '#104E8B', '#8DEEEE'],
                           linestyle=['-', '-', '-', ':', '--'])
        # ax0.plot(x, y, x, y_0, x, y_1, x, y_2, x, y_3, linewidth = 0.8)
        ax0.plot(x, y_0, x, y_1, x, y_2, linewidth=1.0)
        ...
        plt.legend(['SAE-LSTM m=7', 'SAE-LSTM m=12', 'SAE-LSTM m=35'], loc=led_location,
                   fontsize=14)
