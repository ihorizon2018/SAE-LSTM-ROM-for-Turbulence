...
def cc(ori_data, rom_data_0, rom_data_1, rom_data_2, rom_data_3,rom_data_4,rom_data_5,fieldName,startNumber,legLocation):
...
# draw the plot for correlation coefficient
    ...
    if ori_data.ndim == rom_data_0.ndim:
        if rom_data_0.ndim == 3:
            ...
            fig.dpi=200
            ax0 = fig.add_subplot()
            #ax0.set_prop_cycle(color = ['#f6b93b','#FF3030','#104E8B','#8DEEEE','#f8b93b','#FF6030'], linestyle = ['--', '--', '--', '--','--', '--'])
            ax0.set_prop_cycle(color=['#f6b93b', '#FF3030', '#7FFF00', '#BF3EFF', '#0000FF', '#A52A2A'],
                              linestyle=['-', '-', '-', '-', '--', '--'])
            plt.ylim((0, 1.001))
            ...
            if fieldName == 'Velocity':
                ...
                plt.legend(['DMD r=4', 'DMD r=7', 'DMD r=17', 'DMD r=37','DMD r=62', 'DMD r=83'], loc=legLocation,fontsize=15)
            ...
            # plt.legend(['D4:AE+TF', 'D8:AE+TF', 'D12:AE+TF', 'D4:PCA+TF', 'D8:PCA+TF', 'D12:PCA+TF'], loc='lower right') 
                ...
                plt.ylim((0.86, 1.003))
                plt.xlabel('Time(1000s)',{'size' : 15})
                #ax0.xaxis.set_major_formatter(FormatStrFormatter('%.0f'))
                x = np.linspace(startNumber+0, startNumber+(pcc_0.shape[0]-number) / rate, pcc_0.shape[0]-number)[1:]
                ax0.plot(x, y_0, x, y_1, x, y_2, x, y_3,x, y_4,x, y_5,linewidth=1.5)
                plt.legend(['DMD r=3', 'DMD r=4', 'DMD r=7', 'DMD r=10','DMD r=14', 'DMD r=19'], loc=legLocation,fontsize=15)
            plt.show()
    else:
        print('the dimension of these two series are not equal. Please check them.')
...
def point_over_time(ori_data, rom_data_0, rom_data_1, rom_data_2, rom_data_3,rom_data_4, rom_data_5, pointNo, fieldName):
 # rom_data_2, rom_data_4, rom_data_5, pointNo, fieldName):# 1: full model; 2: ROM model
    ...
    if fieldName == 'Velocity': # flow_past_cylinder
        ...
        ax0.set_prop_cycle(color = ['#2f3542','#f6b93b', '#FF3030', '#7FFF00', '#BF3EFF', '#0000FF', '#A52A2A'], linestyle = ['--',':','--', ':', '--', ':','--'])
        # ax0.plot(x, y, x, y_0, x, y_1, x, y_2, x, y_3, linewidth = 0.8)
        ax0.plot(x, y, x, y_0, x, y_1, x, y_2, x, y_3,x, y_4,x, y_5)
        ...
        plt.xlabel('Time(s)')
        plt.ylabel(fieldName + ' Magnitude')
        # plt.legend(['Full Model', 'ROM(AE+TF)', 'ROM(PCA+TF)'], loc='lower right')
        plt.legend(['Full Model', 'DMD r=62', 'AEDMD r&dim=62', 'AEDMD r&dim=37', 'AEDMD r&dim=17', 'AEDMD r&dim=7', 'AEDMD r&dim=4'], loc='lower right')
        ...
        #point = [2900,3150] # [start point, finish point]->200s
        ...
        # time = int((point[1]-point[0])/rate)
        # x = np.linspace(point[0],point[1],time*rate) 
        x = np.linspace(0,rom_data_0[1]/rate,rom_data_0.shape[0])
        # y_u = ori_data[point[0]:point[1],pointNo,0] # u
        # y_v = ori_data[point[0]:point[1],pointNo,1] # v
        # y_0_u = rom_data_0[point[0]:point[1],pointNo,0] # u
        # y_0_v = rom_data_0[point[0]:point[1],pointNo,1] # v
        ...
        plt.xlabel('Time(s)',{'size' : 11})
        plt.ylabel('Velocity',{'size' : 11})
       
        plt.legend(['Full Model', 'DMD', 'AEDMD dim=50', 'AEDMD dim=40', 'AEDMD dim=30'], loc='upper left')
...
def rmse_over_time(ori_data, rom_data_0, rom_data_1, rom_data_2, rom_data_3,rom_data_4,rom_data_5,fieldName,startNumber,legLocation):
    if fieldName == 'Velocity':
        ...
        #print(x)
        ax.set_prop_cycle(color = ['#f6b93b','#FF3030','#7FFF00','#BF3EFF','#0000FF','#A52A2A'], linestyle = ['-', '-', '-', '-','-', '-'])
        ...
        # plt.title('Correlation Coefficient')
        ax.plot(x, y_0, x, y_1, x, y_2, x, y_3, x, y_4, x, y_5,linewidth = 1.5)
        ...
        #print(x)
        ax0.set_prop_cycle(color = ['#f6b93b','#FF3030','#7FFF00','#BF3EFF','#0000FF','#A52A2A'], linestyle = ['-', '-', '-', '-','-', '-'])
        #x = np.linspace(0,point[1]/rate,int(point[1]-point[0]))
        # x = np.linspace(0,15,2875)
        ...
        plt.ylim((-0.00006, 0.0025))
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)
        ax0.yaxis.set_major_formatter(FormatStrFormatter('%.4f'))
        #ax0.xaxis.set_major_formatter(FormatStrFormatter('%.0f'))
